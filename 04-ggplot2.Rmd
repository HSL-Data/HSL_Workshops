...
## Resources to learn ggplot2
*Chapter 3 in [R 4 Data Science](http://r4ds.had.co.nz/)
*[R Graphics Cookbook](http://www.cookbook-r.com/Graphs/)
*Show the Help menu cheat sheet

This lesson assumes a [basic familiarity with R](IntroR.html) and [manipulating data with dplyr and `%>%`](TamingWildData.html).

## Make sure everyone has tidyverse or readr, dplyr, ggplot2
## Download data and make a new project

### Gapminder data

We're going to work with a [cleaned-up excerpt](https://github.com/jennybc/gapminder) from the [Gapminder data](http://www.gapminder.org/data/). Download the [**gapminder.csv** data by clicking here](data/gapminder.csv) or using the link above.
...
# Read in downloaded usind readr package
...
Additionally, the **`%>%`** operator allows you to "chain" operations together. Rather than nesting functions inside out, the `%>%` operator allows you to write operations left-to-right, top-to-bottom. 


Let's say we wanted to get the average life expectancy and GDP (not GDP per capita) for Asian countries for each year.
...
![](img/nest_vs_pipe.png)

## About ggplot2 {#ggplot}
...
Now tweak the parameters of the line to show a thick red line with no standard error stripes
...
But notice what happens next: if we make the call to `aes()` *outside* of the `geom_point()` and `geom_smooth` calls, then the continent variable gets mapped as an aesthetic to all the following geoms. So here, we get separate smoothing lines for each continent.
...
p + geom_point() #A
p + geom_point() + facet_wrap(~continent) #B
p + geom_point() + geom_smooth() #C
p + geom_point() + geom_smooth() + facet_wrap(~continent) #C
```
...
p + geom_line(aes(group=country), alpha=.5) + geom_smooth(lwd=3, se=FALSE) +
...
Reorder was operating on the mean of the raw variable. We need a mean of the log10 transformed gdpPercap
...
p + geom_density(aes(fill=continent), alpha=1/4) #C

p + geom_histogram(aes(fill=continent)) + facet_wrap(~continent, ncol=1) #D
ggsave("myplot.pdf", width=6, height=10) #E
```
...
Finally, to add custom colors use scale_color_manual
```{r}
p + theme_tufte() + scale_color_manual(values = c("red", "orange", "yellow", "green", "blue"))
```
