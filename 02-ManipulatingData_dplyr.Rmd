...
[nhanes data set](data/nhanes.csv)
...
You will only need to install the package once per computer, so that is why we do not preserve this line in the script. 
...
Let's install and load a second package called dplyr which has functions for manipulating datasets.
...
Then load the package by writing this line in your script

```{r}
library("dplyr")
```

Notice the messages that come out. The function ran and there is no error, but there is some text in red that we should read and understand. 
...
## Resources 
- [_Introduction_(10.1)](http://r4ds.had.co.nz/tibbles.html#introduction-4) and [_Tibbles vs. data.frame_ (10.3)](http://r4ds.had.co.nz/tibbles.html#tibbles-vs.data.frame) sections of the [**_R for Data Science_ book**](http://r4ds.had.co.nz/tibbles.html). 
